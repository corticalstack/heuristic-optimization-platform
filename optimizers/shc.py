...
class SHC(Optimizer):
    ...
    def stochastic_hill_climbing(self):
        self.gbest.perm = getattr(self.problem, 'generator_' + self.cfg.settings['opt']['SHC']['generator'])(self.problem.n)
        self.gbest.fitness, self.budget = self.problem.evaluator(self.gbest.perm, self.budget)

        while self.budget > 0:
            new = Particle()
            new.perm = self.n_swap(self.gbest.perm)  # One neighbour at random by simple pairswap
